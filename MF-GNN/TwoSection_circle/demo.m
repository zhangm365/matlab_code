...
% 有损
aviobj = VideoWriter('result\triangle.avi');
open(aviobj);
for k=1:nframes
    ...
    % tracking
    subplot(1,3,1);
    plot3(rdx*1000, rdy*1000, rdz*1000,'k-','color',[0.75,0.75,0.75]);hold on;
    plot3(pos(1:10:x1,1)'*1000, pos(1:10:x1,2)'*1000, pos(1:10:x1,3)'*1000,'r-','linewidth',2);
    % 画坐标轴
...
%     grid on
    ...
    if k == 1
        imwrite(imind, cm, 'result/triangle.gif', 'Loopcount', inf, 'DelayTime', 0.001);
    else
        imwrite(imind,cm, 'result/triangle.gif','WriteMode','append','DelayTime',0.001);
    end
    % 保存avi文件
