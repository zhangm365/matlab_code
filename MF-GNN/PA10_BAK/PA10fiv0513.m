...
figure;
plot3(j7px,j7py,j7pz);hold on;
%     joints01=line('xdata',[j0px(j);j1px(j)],'ydata',[j0py(j);j1py(j)],'zdata',[j0pz(j);j1pz(j)],...
%         'color', 'red','erasemode','none');
for j=1:length(t),
    reach=0.315;
 	joints01=line('XData', [0;0], 'YData', [0;0], 'ZData', [-reach;0], 'color', 'magenta','erasemode','none');
...
p=1;
 ...
 hr=line('color', 'green', 'erasemode', 'xor');
mag=0.2*reach;
