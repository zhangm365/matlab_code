...
figure;
Te = 0:step:t(number);
p1 = plot(Te,Error(:,1)'*1000,'c-.', 'linewidth', 2);
hold on;
p2 = plot(Te,Error(:,2)'*1000,'m--', 'linewidth', 2);%grid on;
p3 = plot(Te,Error(:,3)'*1000,'b:', 'linewidth', 2);

RMSE = round(sqrt(sum(Error(:,1).^2 + Error(:,2).^2 + Error(:,3).^2)/number), 4);
annotation = strcat('RMSE=',mat2str(RMSE),' mm');
text(5, -0.4, annotation, 'fontsize', 15,'FontName', 'times new Roman');
