...
figure;
plot3(rd(1,1:interval:steps), rd(2,1:interval:steps), rd(3,1:interval:steps), 'r','linewidth', 2);grid on;
hold on;
plot3(ra(1,1:interval:steps), ra(2,1:interval:steps), ra(3,1:interval:steps), 'b:*','linewidth', 1);
hold off;
axis equal
% legend('Desired path', 'Actual trajectory', 'best', 'FontName', 'times new Roman', 'fontsize', 18);
legend('期望轨迹', '实际轨迹', 'best', 'fontsize', 12);
set(gca,'FontSize', 18);
...
legend('Ex','Ey','Ez','Location','best', 'FontName', 'times new Roman','fontsize',12);
RMSE = round(sqrt(sum(Error(1,:).^2 + Error(2,:).^2 + Error(3,:).^2)/steps)*1000, 4);
annotation = strcat('RMSE=',mat2str(RMSE),' mm');
text(6.5, -2e-4, annotation, 'fontsize', 12, 'FontName', 'times new Roman');
hold off;
set(gca,'FontSize',18);
xlabel('Time (s)', 'FontName', 'times new Roman','fontsize',20);
ylabel('Error (mm)', 'FontName', 'times new Roman','fontsize',20);
